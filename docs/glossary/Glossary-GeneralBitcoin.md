...
The Bitcoin Mempool (memory pool) is a collection of all transaction data that has been verified by Bitcoin nodes, but has not yet confirmed in a block.
...
A non-custodial wallet is any wallet where the private keys are generated by the user.
Non-custodial wallets give you full control over your bitcoins.

Wasabi eliminates the risk that the coordinator can steal your bitcoin.
Each participant verifies the proposed transaction outputs before signing.
This signature is only valid for this transaction, so any attempt to change the transaction after it is signed would be rejected.
