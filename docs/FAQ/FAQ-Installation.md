...
In the terminal, change the directory to the one with the downloaded files, and verify the signature with `gpg --verify Wasabi-1.1.8.deb.asc`.
...
Now install Wasabi with `sudo dpkg -i Wasabi-1.1.8.deb.asc`, and run it with `wassabee`.
